...
class NewOkHttpSetup : IOkHttpSetup {
...
//        EkaApp.instance.applicationContext.startActivity(intent)
    ...
    private suspend fun refreshToken(): String? {
        val request = RefreshSessionRequest(
            refresh = UserSharedPref(EkaApp.instance).getRefreshToken() ?: "",
            sessionToken = UserSharedPref(EkaApp.instance).getSessionToken() ?: ""
        )
        val userService: UserServiceEmr = Networking.create(
            UserServiceEmr::class.java,
            BuildConfig.EKA_CARE_AUTH_GO_URL
        )
